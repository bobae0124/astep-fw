...
##################### ASIC METHODS FOR USERS #########################
    ...
    #async def enable_pixel(self, row: int, col: int, inplace:bool=True):
    #   self.asic.enable_pixel(col, row, inplace=True)
       #await self.asic_update()
##    #Interface with asic.py   from astropix.py, row<->col
##    def enable_pixel(self, row: int, col: int, inplace:bool=True):
##       self.asic.enable_pixel(col, row, inplace)
##
    async def disable_pixel(self, row: int, col: int):
       self.asic.disable_pixel(col, row)
...
# add by me
#    def extract_segments(data, prefix='0a0104', segment_length=22):
#        segments = []
#        start = 0
#        
...
#        return segments
    ...
    def decode_readout(self, readout:bytearray, i:int, printer: bool = True, nmb_bytes:int = 11):
        ...
        short = 0
        #list_hits = [readout[i:i+nmb_bytes] for i in range(0,len(readout),nmb_bytes)] # orig.

# 2024.June.2 updated '0a0104'
        list_hits = []
        start = 0
        prefix='0a0104'
        segment_length=22
        ...
        print(f"list_hits = {list_hits}")

        hit_list = []
        #print(f"*********************** printout list_hits = {list_hits}")
        ...
        for hits in list_hits:  # 2024.June.2 update
            ...
            if len(hit) < 3:
                short += short
                print(f"{short}---- ???  empty ???   len(hit) = {len(hit)}...{hit[0]}  {bytearray(hit)} ------------------------ << << ")
                continue 
            #print(f"***! sum(hit) == {sum(hit)} or hit[0] = {hit[0]}, hit[1] = {hit[1]}, int(hit[0])+int(hit[1]) == {(int(hit[0])+int(hit[1]))} !***")
            #if (sum(hit) == 1020) or (int(hit[0])+int(hit[1]) == 510): #HARDCODED MAX BUFFER or 'HIT' OF ONLY 1'S- WILL NEED TO REVISIT
            if (int(hit[0])+int(hit[1]) == 510) or (int(hit[0])+int(hit[1]) == 15): #HARDCODED MAX BUFFER or 'HIT' OF ONLY 1'S- WILL NEED TO REVISIT
                short += short
                print(f"{short}=====  hit[0]={int(hit[0])}, hit[1]={int(hit[1])}: hit[0]+hit[1] == 510 [ffff] or hit[0]+hit[1] == 15 [0a0104]  =====")
                continue 
            try:
                id          = int(hit[2]) >> 3
