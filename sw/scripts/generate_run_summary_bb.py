...
#        all_files += nfile
    ...
    if 'readout' in df.columns:
        if len(df['readout']) > 0:
            ...
            print("check: The 'readout' column is empty.")
    else:
        print("check: The 'readout' column does not exist.")
            ...
            for indc in dffcol.index:
				...
				#col0-3 skip
                    ...
                    if (abs(dffcol['timestamp'][indc] - dffrow['timestamp'][indr]) < timestamp_diff) & (abs(dffcol['tot_us'][indc] - dffrow['tot_us'][indr])/dffcol['tot_us'][indc]*100 < tot_time_limit):
                        ...
                        dffrow = dffrow.drop(indr) # matched row, remove next for loop 
                        break
...
#    print(f"{len(pixs)}, {npixel}% active")
    ...
    dffpair = pd.DataFrame(pair, columns=['readout','col', 'row', 
