...
class UARTIO(rfg.core.RFGIO):
    ...
    def writeBytesIO(self,bytes):
        remaining = len(bytes)
        while remaining > 0:
            #logger.debug("Writing %d bytes to UART",len(bytes))
            written = self.serialPort.write(bytes)
    ...
    async def writeBytes(self,bytes : bytearray):
        try:
            result = await asyncio.get_running_loop().run_in_executor(None, partial(self.writeBytesIO,bytes=bytes))
