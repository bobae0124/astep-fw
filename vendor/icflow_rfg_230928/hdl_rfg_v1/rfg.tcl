    ...
    proc registersToDict registers {
        ...
        #while {$remaining >= 0 } 
            ...
            if {[icflow::args::contains $args -fifo*slave] && [icflow::args::contains $args -read_count]} {
                set registers [linsert $registers [expr $index+1] [list ${name}_read_size -size 32 -sw_read_only -hw_write]]
                incr remaining 1
            } 

            if {[icflow::args::contains $args -fifo*master] && [icflow::args::contains $args -write_count]} {
                set registers [linsert $registers [expr $index+1] [list ${name}_write_size -size 32 -sw_read_only -hw_write]]
                incr remaining 1
            }    
